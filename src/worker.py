...
class WorkerThread:
    def __init__(self, 
                 thread_id: int, 
                 horde: HordeAPI, 
                 backend: BackendAdapter,                  config: Any, 
                  stats: WorkerStats,
                  health_monitor: Any,
    ...
    async def run(self):
        ...
        while not self.shutdown_event.is_set():
            # 0. Check paused execution
            if getattr(self.aggregator, "paused", False):
                try:
    ...
    async def _process_job(self, job: HordeJob):
        ...
        try:
            ...
            if self.aggregator:
                self.aggregator.set_active(self.thread_id, job.id, job.model, context_len, requested_tokens, "Generating")
            ...
            if self.aggregator:
                self.aggregator.update_status(self.thread_id, "Submitting")
            ...
            if kudos is not None:
                ...
                if self.aggregator:
                    self.aggregator.complete_job(self.thread_id, result.token_count or (len(text) // 4), kudos, generation_time)
